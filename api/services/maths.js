...
const Maths = {
	...
	divergenceBeta: (alpha_1, beta_1, alpha_2, beta_2) => {
		return (
			betaln(alpha_2, beta_2) - betaln(alpha_1, beta_1)
	...
	update: (alpha, beta, mu_winner, sigma_sq_winner, mu_loser, sigma_sq_loser) => {
		const updated_annotator = Maths.updatedAnnotator(alpha, beta, mu_winner, sigma_sq_winner, mu_loser, sigma_sq_loser);
		const updated_mus = Maths.updatedMus(alpha, beta, mu_winner, sigma_sq_winner, mu_loser, sigma_sq_loser);
		const updated_sigma_sqs = Maths.updatedSigmaSqs(alpha, beta, mu_winner, sigma_sq_winner, mu_loser, sigma_sq_loser);
	...
	updatedMus: (alpha, beta, mu_winner, sigma_sq_winner, mu_loser, sigma_sq_loser) => {
		const mult = (alpha * exp(mu_winner)) / (alpha * exp(mu_winner) + beta * exp(mu_loser)) - (exp(mu_winner)) / (exp(mu_winner) + exp(mu_loser));
	...
	updatedAnnotator: (alpha, beta, mu_winner, sigma_sq_winner, mu_loser, sigma_sq_loser) => {
		const c_1 = (
			exp(mu_winner) / (exp(mu_winner) + exp(mu_loser)) + 0.5
		...
		const expt = (c_1 * (alpha + 1) * alpha + c_2 * alpha * beta) / (c * (alpha + beta + 1) * (alpha + beta));
		const expt_sq = (
