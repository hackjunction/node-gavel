		...
		if (teamMember.email && teamMember.name) {
			this.setState({
				requestBody: {
					...
					teamMembers: this.state.requestBody.teamMembers ? _.concat(this.state.requestBody.teamMembers, teamMember) : [teamMember]
				}
			});
			...
			<div className="Test--section-wrapper">
				...
				<div className="Test--container">
					...
					<input
						...
						onChange={(event) => this.setState({ requestBody: { ...this.state.requestBody, name: event.target.value } })}
					/>
					<input
						className="Test--input"
						placeholder="Team location"
						onChange={(event) => this.setState({ requestBody: { ...this.state.requestBody, location: event.target.value } })}
					/>
